...
class RequestManager {
    ...
    static func sendRequest(send request: Request, from customer: Customer, to designer: Designer) {
        let ref = Database.database().reference().child("request").child("kevin")
        let requestRef = ref.childByAutoId()
        let value = ["customer": "elephant", "date": "2017-12-25 13:00", "service": "hair care", "check": false] as [String : Any]
